...
io.on('connection', function(socket){
  ...
  // Pour connaître quand quelqu'un se déconnecte
  ...
  socket.on('chat', (message) => {
    ...
    // deux façons d'envoyer un message : 
    io.emit('chat', message); // pour tout le monde, y compris l'envoyeur

    //2. socket.broadcast.emit(message); // pour tout le monde à part l'envoyeur
  });
});
